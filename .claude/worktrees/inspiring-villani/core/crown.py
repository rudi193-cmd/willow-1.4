...
Witness DB: crown_witness_log table in Postgres (scoped to user schema)
...
def _connect(username: str = None):
    ...
    conn = get_connection(schema=username) if username else get_connection()
...
def init_witness(username: str = None) -> None:
    """Create witness log table. Idempotent."""
    conn = _connect(username)
    try:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS crown_witness_log (
        ...
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_wlog_user ON crown_witness_log(username, produced_at)"
        )
        conn.commit()
    finally:
        conn.close()
...
def artifact(content: str, title: str, source_type: str,
             username: str, loam_db_path: str = None,
...
def record(title: str, content: str, agent: str,
    ...
    try:
        conn.execute(
            """INSERT OR IGNORE INTO crown_witness_log
...
def verify_witness(witness_id: str, content: str,
    ...
    try:
        row = conn.execute(
            "SELECT content_hash FROM crown_witness_log WHERE witness_id=?",
            (witness_id,)
        ).fetchone()
...
def get_witness_log(username: str, limit: int = 20) -> list:
    """Return recent witness records for a user."""
    conn = _connect(username)
    try:
        rows = conn.execute(
            "SELECT witness_id, agent, title, content_hash, content_len, produced_at "
            "FROM crown_witness_log WHERE username=? ORDER BY produced_at DESC LIMIT ?",
            (username, limit)
        ).fetchall()
