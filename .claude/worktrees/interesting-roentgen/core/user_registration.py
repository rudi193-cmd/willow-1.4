...
from core.db import get_connection, init_user_schema
...
def init_user_knowledge_db(username: str) -> None:
    """Create knowledge schema in Postgres for this user (lattice columns included)."""
    # Ensure user schema exists in Postgres
...
def create_user_config(username: str, display_name: str, trust_level: int) -> dict:
    ...
    config = {
        ...
        "knowledge_db": "postgres",  # knowledge lives in Postgres user schema
...
def register_user(username: str, display_name: str, trust_level: int = 1) -> None:
    ...
    print(f"  DB:         postgres (schema={username})")
